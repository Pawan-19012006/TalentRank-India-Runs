...
router.post('/copilot/query', (req, res) => {
  ...
  const candidates = (contextCandidates && contextCandidates.length > 0) ? contextCandidates : getDossiers();
  ...
  const matchedCandidate = candidates.find(c => 
    c.name && (lowerQuery.includes(c.name.toLowerCase()) || 
    lowerQuery.split(' ').some(word => word.length > 3 && c.name.toLowerCase().includes(word)))
  );

  if (matchedCandidate) {
    const rank = candidates.findIndex(c => c.id === matchedCandidate.id) + 1;
    responseText = `Yes, I found **${matchedCandidate.name}**. They are currently ranked #${rank} with a match score of **${matchedCandidate.score}%**. They are a **${matchedCandidate.role}** based in ${matchedCandidate.loc}.`;
  }
  else if (lowerQuery.includes('rag') || lowerQuery.includes('vector') || lowerQuery.includes('faiss')) {
    const topRag = [...candidates].sort((a, b) => (b.score || 0) - (a.score || 0))[0];
    if (topRag) {
      responseText = `I found strong RAG candidates in the pool. The top match is **${topRag.name}** with a match score of **${topRag.score}%**. They have strong AI signals. Would you like to review their profile?`;
    }
  } else if (lowerQuery.includes('notice') || lowerQuery.includes('join') || lowerQuery.includes('days')) {
    const fastJoiner = candidates[2] || candidates[0];
    if (fastJoiner) {
      responseText = `Several candidates are immediately available. For example, **${fastJoiner.name}** is available within 30 days. They are currently a **${fastJoiner.role}** and have a match score of **${fastJoiner.score}%**.`;
    }
  } else if (lowerQuery.includes('why') || lowerQuery.includes('top match') || lowerQuery.includes('ranked #1')) {
    const topCand = candidates[0];
    if (topCand) {
      responseText = `**${topCand.name}** is ranked #1 because they have:
- An exceptional skills score of **${topCand.skills}%**
- Extensive domain experience (**${topCand.experience}%**)
- High behavioral & culture fit (**${topCand.behavior}%**).`;
    }
  }
